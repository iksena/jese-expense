...
import { AlertTriangle, Calendar, ChevronLeft, ChevronRight, CreditCard, Dumbbell, Edit2, Filter, Loader2, LogOut, Plus, RefreshCw, Search, Settings, Trash2, TrendingUp, Users, Wallet, X } from 'lucide-react';
...
export default function ExpenseDashboard({ 
  ...
  const filteredExpenses = useMemo(() => {
    return expenses.filter(exp => {
      ...
      const spenderMatch = filterSpender === 'All' || exp.spender === filterSpender;
      
      // General search query - search across multiple fields
      const query = searchQuery.toLowerCase().trim();
      const searchMatch = !query || 
        exp.description.toLowerCase().includes(query) ||
        exp.category.toLowerCase().includes(query) ||
        exp.spender.toLowerCase().includes(query) ||
        exp.amount.toString().includes(query) ||
        exp.currency.toLowerCase().includes(query) ||
        exp.date.includes(query) ||
        formatDateFriendly(exp.date).toLowerCase().includes(query) ||
        formatCurrency(exp.amount, exp.currency).toLowerCase().includes(query);

      return monthMatch && categoryMatch && spenderMatch && searchMatch;
    });
  }, [expenses, selectedMonth, filterCategory, filterSpender, searchQuery]);
  ...
  const hasActiveFilters = filterCategory !== 'All' || filterSpender !== 'All' || searchQuery !== '';
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex-1">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Search by description, amount, date, category..."
              ...
              <div className="flex items-center gap-2">
                ...
                <select 
                  ...
                  className="px-4 py-2 border rounded-lg bg-white focus:ring-2 focus:ring-purple-500 outline-none"
                >
              ...
              <div className="flex items-center gap-2">
                ...
                <select 
                  value={filterSpender} 
                  onChange={(e) => setFilterSpender(e.target.value as Spender | 'All')} 
                  className="px-4 py-2 border rounded-lg bg-white focus:ring-2 focus:ring-purple-500 outline-none"
                >
                  <option value="All">All Spenders</option>
                  <option value="User 1">{settings.user1name}</option>
                  <option value="User 2">{settings.user2name}</option>
                  <option value="Together">Together</option>
                </select>
              </div>
              ...
              {hasActiveFilters && (
                <button
                  ...
                  className="px-4 py-2 text-sm font-medium text-purple-600 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition-colors flex items-center gap-2 whitespace-nowrap"
                >
                  <X className="w-4 h-4" />
                  Clear Filters
                </button>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {filterSpender !== 'All' && (
                  <span className="px-3 py-1 bg-blue-100 text-blue-700 text-sm rounded-full flex items-center gap-2">
                    Spender: {filterSpender === 'User 1' ? settings.user1name : filterSpender === 'User 2' ? settings.user2name : 'Together'}
                    <button onClick={() => setFilterSpender('All')} className="hover:text-blue-900">
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                )}
